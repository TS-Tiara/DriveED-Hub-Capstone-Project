...
<div class="container mx-auto px-4 py-6 max-w-5xl">
    <div class="mb-6">
        <a href="{{ school_route('student.courses.modules.index', ['course' => $course->id]) }}" class="text-sm text-blue-600 hover:underline">← Back to Modules</a>
    </div>
        ...
        <ul class="divide-y border rounded-md">
            @forelse(($module->lessons ?? collect()) as $lesson)
                <li class="p-3 flex items-center justify-between text-sm">
                    ...
                    <a href="{{ school_route('student.courses.modules.lessons.show', ['course' => $course->id, 'module' => $module->id, 'lesson' => $lesson->id]) }}" class="text-blue-600 hover:underline">Open</a>
                </li>
            @empty
                <li class="p-3 text-sm text-gray-500">No lessons are available for this module yet.</li>
            @endforelse
        </ul>
    </div>
</div>
