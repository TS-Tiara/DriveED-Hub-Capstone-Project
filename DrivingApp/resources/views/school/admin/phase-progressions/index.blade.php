...
<style>
    ...
    .stat-value { color: #111827; font-weight: 700; font-size: 1.3rem; margin-top: 4px; }
    ...
    .btn-apply { background: {{ $primaryColor }}; color: #fff; }
    .btn-clear {
        background: #f3f4f6;
        color: #374151;
...
<div class="page-wrap">
    ...
    <form class="filters" method="GET" action="{{ route('schools.admin.phase-progressions.index', ['school' => $school->slug]) }}">
        <div>
        ...
        <div style="display:flex; gap:8px; align-items:end;">
            ...
            <a class="btn btn-clear" href="{{ route('schools.admin.phase-progressions.index', ['school' => $school->slug]) }}">Clear</a>
        </div>
    </form>
        ...
        <table>
            ...
            <tbody>
                @forelse($progressions as $progression)
                    <tr>
                        <td>{{ $progression->enrollment->student->name ?? $progression->enrollment->learner->name ?? 'N/A' }}</td>
                        ...
                        <td>
                            <span class="status-badge status-{{ $progression->status }}">{{ ucfirst($progression->status) }}</span>
                        </td>
                        ...
                        <td>
                            @if($progression->status === 'pending')
                                <div class="actions">
                                    <form method="POST" action="{{ route('schools.admin.phase-progressions.approve', ['school' => $school->slug, 'phaseProgression' => $progression->id]) }}">
                                        @csrf
                                        <input type="hidden" name="admin_notes" value="Approved by admin">
                                        <button type="submit" class="btn btn-approve">Approve</button>
                                    </form>

                                    <form method="POST" action="{{ route('schools.admin.phase-progressions.reject', ['school' => $school->slug, 'phaseProgression' => $progression->id]) }}" style="display:flex; gap:8px;">
                                        @csrf
