...
<div class="admin-container">
    <div class="page-header">
        <div class="page-header-left">
            <h1 class="page-title">Edit Student Progress</h1>
            <p class="page-subtitle">Update progress for {{ $progress->student->name ?? 'Student' }}</p>
        </div>
    </div>

    <div class="form-card">
        <form action="{{ $schoolRoute('instructor.progress.update', $progress->id) }}" method="POST">
            @csrf
            @method('PUT')
            
            <div class="form-group">
                <label>Student</label>
                <input type="text" value="{{ $progress->student->name ?? 'Unknown' }}" disabled>
            </div>

            <div class="form-group">
                <label>Course</label>
                <input type="text" value="{{ $progress->course->title ?? 'Unknown' }}" disabled>
            </div>

            <div class="form-group">
                <label for="completion_percent">Completion Percentage</label>
                <input type="number" name="completion_percent" id="completion_percent" 
                       min="0" max="100" value="{{ $progress->completion_percent }}" required>
            </div>

            <div class="form-group">
                <label for="notes">Notes (Optional)</label>
                <textarea name="notes" id="notes" rows="4">{{ $progress->notes }}</textarea>
            </div>
