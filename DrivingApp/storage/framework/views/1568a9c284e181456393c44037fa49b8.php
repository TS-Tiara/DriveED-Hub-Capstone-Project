...
<script>
    console.log('Instructor schedule script loaded');
    let inlineCalendarVisible = false;
    let pendingSlotFormId = null;
    
    // Toggle timeslot (select or leave) via fetch - like student schedule does
    function selectTimeSlot(slotId, button) {
        console.log('selectTimeSlot called with slotId:', slotId);
        alert('Select slot clicked: ' + slotId);
        ...
        .then(data => {
            if (data.success) {
                showSlotNotification(data.message || 'Slot selected successfully!', 'success');
    ...
    function leaveTimeSlot(slotId, button) {
        console.log('leaveTimeSlot called with slotId:', slotId);
        alert('Leave slot clicked: ' + slotId);
        ...
        .then(data => {
            if (data.success) {
                showSlotNotification(data.message || 'Left slot successfully!', 'success');
                setTimeout(() => window.location.reload(), 1000);
            } else {
                showSlotNotification(data.message || 'Failed to leave slot', 'error');
                button.textContent = originalText;
                button.disabled = false;
            }
        })
    ...
    // Time slot buttons now use onclick handlers with fetch() like student page
