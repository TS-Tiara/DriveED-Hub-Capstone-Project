...
<head>
    ...
    <style>
        ...
        .grade-badge { padding: 2px 8px; border-radius: 3px; font-size: 10px; font-weight: 600; }
    ...
    <table>
        ...
        <tbody>
            ...
            @php
                ...
                $gradeClass = $grade >= 80 ? 'grade-high' : ($grade >= 60 ? 'grade-mid' : 'grade-low');
            @endphp
            <tr>
                <td>{{ $index + 1 }}</td>
                <td>{{ $student->name }}</td>
                ...
                <td>{{ $student->upcoming_sessions }}</td>
                <td>
                    @if($grade)
                        <span class="grade-badge {{ $gradeClass }}">{{ number_format($grade, 1) }}</span>
                    @else
                        N/A
                    @endif
                </td>
