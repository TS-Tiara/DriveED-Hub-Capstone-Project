...
<div class="lms-page" style="max-width: 850px;" data-breadcrumb-course="{{ $course->title ?? '' }}" data-breadcrumb-module="{{ $module->title ?? '' }}">
    <div class="lms-header">
        ...
        <div class="lms-actions">
            <a href="{{ school_route('instructor.courses.modules.show', ['course' => $course->id, 'module' => $module->id]) }}" class="lms-btn lms-btn-muted">Cancel</a>
        </div>
    </div>
    ...
    <div class="lms-form-wrap">
        ...
        <form method="POST" action="{{ school_route('instructor.courses.modules.update', ['course' => $course->id, 'module' => $module->id]) }}" class="lms-form">
            @csrf
            @method('PUT')
            ...
            <div class="lms-actions" style="margin-top: 4px;">
                ...
                <a href="{{ school_route('instructor.courses.modules.show', ['course' => $course->id, 'module' => $module->id]) }}" class="lms-btn lms-btn-muted">Back</a>
            </div>
        </form>
        </div>
</div>
